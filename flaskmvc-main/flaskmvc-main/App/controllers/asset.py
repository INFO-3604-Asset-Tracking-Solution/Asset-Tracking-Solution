...
def add_asset(description, brand, model, serial_number, cost, notes, status_name="Available"): 
    try:
        ...
        if status_name:
            status = AssetStatus.query.filter_by(status_name=status_name.strip()).first()        

        if not status:
            print(f"Invalid status: {status_name}")
            status = AssetStatus.query.filter_by(status_name="Available").first()

            if not status:
                print("CRITICAL: 'Available' status does not exist in DB")
                return None
        ...
        db.session.rollback()
        print(f"Error: Invalid asset status '{status_name}'.")
        return None

    except IntegrityError: # Catch specific duplicate key error
        db.session.rollback()
        print(f"Error: Duplicate Asset")
        return None
