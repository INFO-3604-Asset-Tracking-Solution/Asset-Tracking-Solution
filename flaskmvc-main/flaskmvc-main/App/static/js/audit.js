...
async function ensureActiveAuditSession() {
     ...
     * @param {string} scanMethod - The method used (manual, barcode, qrcode, rfid)
     */
    async function processScannedAsset(assetId, scanMethod = 'manual') {
        if (!state.isRoomAuditActive) return;

        // Fetch condition from the interactive dropdown (defaults to Good if missing)
        const conditionSelect = document.getElementById('scanConditionSelect');
        const currentCondition = conditionSelect ? conditionSelect.value : 'Good';

        // 1. Log the scan event to the backend (linked to active audit)
        let checkOutcome;
        try {
            checkOutcome = await createCheckEvent(assetId, state.currentRoom, currentCondition);
            
            // Auto reset the condition back to Good after successful log as requested by UI feedback
            if (conditionSelect) conditionSelect.value = 'Good';
            
        } catch (error) {
            // Error handling already done in createCheckEvent
            return;
        }
        ...
        if (assetInRoom) {
            ...
            // Asset not in expected list, check if it's misplaced or unknown
            if (checkOutcome && checkOutcome.success) {
    ...
    /**
     * Creates a CheckEvent in the backend, linking it to the current Audit
     */
    async function createCheckEvent(assetId, foundRoomId, condition) {
        try {
            const response = await fetch('/api/check-event', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                if (data.message === 'No active audit found') {
                    UI.showMessage('Critical: No active Audit session! Please start one first.', 'danger');
                    // Automatically stop room audit if no workspace audit exists
                    stopRoomAudit(false);
                } else {
                    console.error('Check event error:', data.message);
                }
                throw new Error(data.message || 'Failed to create check event');
            }

            return data;
        } catch (error) {
            console.error('Error creating check event:', error);
            throw error;
        }
    }
...
const UI = (function () {
    ...
    function createScannedAssetRow(asset, currentRoom) {
        ...
        const conditionCell = clone.querySelector('.asset-identified-condition');
        if (asset.identifiedCondition === 'Needs Repair') {
            conditionCell.innerHTML = `<span class="badge bg-warning text-dark"><i class="bi bi-tools"></i> Needs Repair</span>`;
        } else if (asset.identifiedCondition === 'Beyond Repair') {
            conditionCell.innerHTML = `<span class="badge bg-danger"><i class="bi bi-x-octagon"></i> Beyond Repair</span>`;
        } else {
            conditionCell.innerHTML = `<span class="badge bg-success"><i class="bi bi-check"></i> Good</span>`;
        }
