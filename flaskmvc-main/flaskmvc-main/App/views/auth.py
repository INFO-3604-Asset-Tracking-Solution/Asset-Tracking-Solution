...
def identify_page():
    return render_template(
        'message.html',
        title="Identify",
        message=f"You are logged in as {current_user.user_id} - {current_user.username}"
    )
...
def login_action():
    ...
    response = jsonify({
        'success': True,
        'redirect': url_for('inventory_views.inventory_page')
    }) if is_ajax else redirect(url_for('inventory_views.inventory_page'))
...
@jwt_required()
def identify_user():
    return jsonify({'message': f"email: {current_user.email}, id: {current_user.user_id}"})
